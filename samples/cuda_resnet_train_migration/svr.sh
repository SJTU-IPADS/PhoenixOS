...
start_server() {
    ...
    if [ $mount = false ] ; then
        $SUDO docker run --gpus all -dit --privileged --network=pos_net \
                        --ip $ip_addr --ipc=host --network=host --name $container_name zobinhuang/pytorch:11.3-ubuntu20.04
        cd $script_dir && cd .. && cd ..
        ...
        $SUDO docker run --gpus all -dit -v $PWD:/root -v /nvme:/nvme --privileged --network=host --ipc=host --name $container_name zobinhuang/pytorch:11.3-ubuntu20.04
