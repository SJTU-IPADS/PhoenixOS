...
int main(){
    ...
    POSOobMsg_MigrationSignal_t migration_signal_msg;
    ...
    // send
    memset(&migration_signal_msg, 0, sizeof(POSOobMsg_MigrationSignal_t));
    migration_signal_msg.msg_type = kPOS_Oob_Migration_Signal;

    len = sizeof(remote);
    retval = sendto(s, &migration_signal_msg, sizeof(POSOobMsg_MigrationSignal_t), 0, (struct sockaddr*)&remote, len);
    if(retval < 0){
        printf_error();
        goto exit;
    }
    fprintf(stdout, "[POS] successfully migration signal\n");
